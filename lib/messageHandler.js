...
async function handleMessage(sock, { messages, type }) {
    try {
        ...
        if (isGroup && Array.isArray(globalDb.antilink) && globalDb.antilink.includes(from)) {
            ...
            if (linkRegex.test(body) && !isSenderAdmin && !isOwner && isBotAdmin) {
                try {
                    ...
                    if (action === "kick") {
                        ...
                        if (kicked) {
                            ...
                            await sock.sendMessage(from, { text: `🚫 *Lien détecté !* @${sender.split("@")[0]} (${count}/3)`, mentions: [sender] });
                        }
                    } else {
                        await sock.sendMessage(from, {
        ...
        if (commandName === "antilink") {
            if (!isOwner && !isSenderAdmin) {
                return await sock.sendMessage(from, { text: "❌ *Access Denied:* Only group admins or the bot owner can use this command." }, { quoted: m });
            }
            const db = readDb();
