...
module.exports = async (sock, m, args) => {
    ...
    if (choice !== "on" && choice !== "off") {
        return await sock.sendMessage(from, { text: "❌ *Usage:* `.antispam on` ou `.antispam off`" }, { quoted: m });
    }
    ...
    group.antispam = (choice === "on");
    writeDb(db);

    await sock.sendMessage(from, {
        text: `⏱️ *AntiSpam:* ${choice === "on" ? "Activé ✅ (plus de 6 messages en 7s = averti/expulsé)" : "Désactivé ❌"}`
    }, { quoted: m });
};
